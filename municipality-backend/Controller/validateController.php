...
if (isset($_POST['submit'])) {
    ...
    if($message!=null){
        ...
        //if email exist or not in mydatabase
        ...
        if($checkNumber && $checkPhone && $checkEmail && $checkName && $checkSurname && $checkPassword && $message==null && !$emailexists) {
            ...
            if ($_POST['select'] == 1) {
                ...
                $adminId = rand(1, 5);
                //object user create
                $cit = new Citizen($name,$adminId, $surname, $address, $password, $phone, $email, $number);

                echo "Creating citizen";
                $query = "INSERT INTO citizen set name= '" . $cit->getName() . "', surname= '" . $cit->getSurname() . "', address='" . $cit->getAddress() . "',
                                    password= '" . $cit->getPassword() . "', phone='" . $cit->getPhone() . "',email= '" . $cit->getEmail() . "',nrMembers= '" . $cit->getNrMembers() . "',admin_id= '" . $cit->getAdminId() . "'";
